...
def search_list(request):
    if 'cname' in request.GET:
        ...
        print(cname)
        # resultList = students.objects.filter(cname=canme) 
        resultList = students.objects.filter(cname__contains=cname) #查詢
    else:
...
def index(request):
    ...
    if not resultList:
        status = False
        errormessage = "No data found."
    # return HttpResponse("Hello, this is the search index page.")
...
def post(request):
    if request.method == 'POST':
        ...
        print(f"Receive POST data: cname={cname}, csex={csex}, cbirthday={cbirthday}, cemail={cemail}, cphone={cphone}, caddr={caddr}")
    ...
    # return HttpResponse("Hello, this is the search post page.")
...
def edit(request, id):
    ...
    if request.method == 'POST':
        ...
        update.save()
        return redirect('index')
        # return HttpResponse("edit 已送出 POST 請求。")
    else:
...
def delete(request, id):
    ...
    if request.method == 'POST':
        ...
        delete_data.delete()
        return redirect('index')
        # return HttpResponse("delete 已送出 POST 請求。")
...
def createItem(request):
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "Fail to create item"}, status=500)
...
def updateItem(request,id):
    ...
    try:
        ...
        try:
            ...
            return JsonResponse({"error": "Fail to update item"}, status=500)
